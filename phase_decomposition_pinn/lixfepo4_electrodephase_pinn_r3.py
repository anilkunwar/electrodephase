    ...
    def uniform_sample(self):
        ...
        if self.geometry == 'cartesian_2d':
            ...
            try:
                ...
                print(f"Sampling error: {e}. Using random sampling instead.")
...
class PhysicsLossCalculator:
    ...
    def __init__(self, constants, geometry='cartesian_2d', device='cpu'):
        ...
        self.geometry = geometry
        self.device = device
        self.scaler = GradScaler() if device == 'cuda' and torch.cuda.is_available() else None
        self.loss_weights = {
...
class TrainingManager:
    ...
    def setup_optimizer(self, lr=1e-3, weight_decay=1e-5, use_lookahead=False):
        ...
        # Learning rate scheduler with warmup - handle older PyTorch versions
        try:
            # Try with verbose parameter for newer PyTorch
            ...
            try:
                ...
                # If that fails too, use a simple scheduler
                print(f"Warning: Could not create ReduceLROnPlateau scheduler: {e}")
                print("Using StepLR scheduler as fallback")
        ...
        for epoch in range(epochs):
            ...
            if use_mixed_precision and self.loss_calculator.scaler is not None:
                try:
                    ...
                    # Fallback to standard backward pass if mixed precision fails
                    print(f"Mixed precision failed: {e}. Falling back to standard training.")
                    total_loss.backward()
                    torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=1.0)
                    self.optimizer.step()
            else:
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        if checkpoint['scheduler_state_dict'] is not None and hasattr(self.scheduler, 'load_state_dict'):
    ...
    if st.session_state.model_trained and st.session_state.model is not None:
        ...
        with tab3:
            ...
            with col3:
                ...
                try:
                    validation_metrics = {
                        'Interface Width (nm)': interface_width_nm if 'interface_width_nm' in locals() else 1.0,
                        'Mass Conservation (%)': mass_change_pct if 'mass_change_pct' in locals() else 0.0,
                        'Phase Purity': max(phase_FePO4, phase_LiFePO4) if 'phase_FePO4' in locals() and 'phase_LiFePO4' in locals() else 0.95,
