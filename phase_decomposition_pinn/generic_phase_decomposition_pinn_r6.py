...
# Default parameters (reduced for resource efficiency)
DEFAULT_PARAMS = {
    'nx': 128,
    'ny': 128,
    'Lx': 60.0,
    ...
    'lr': 1e-3,
    'pde_points': 2000,  # Reduced points
    'ic_points': 500,    # Reduced points
}
...
@njit(fastmath=True, parallel=False)  # Disabled parallel for stability
...
@njit(fastmath=True, parallel=False)  # Disabled parallel
...
class CahnHilliardPINN(nn.Module):
    ...
    def __init__(self, Lx: float, Ly: float, T_max: float,
        ...
        # Network architecture: Periodic embedding + MLP (reduced size)
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        inputs = torch.cat([x_sin, x_cos, y_sin, y_cos, t_norm], dim=1).float()  # Ensure float32
        c = self.net(inputs)
        return c
    ...
    for epoch in range(epochs):
        ...
        try:
            ...
            for i in range(0, pde_points, batch_size):
                ...
                residual = pde_residual(model, x_batch, y_batch, t_batch)
                pde_loss += torch.mean(residual**2) / num_pde_batches
            
            # IC loss with batching
            ic_loss = 0.0
            ...
            for i in range(0, ic_points, batch_size):
                ...
                ic_loss += initial_condition_loss(model, x_batch, y_batch) / num_ic_batches
            ...
            logger.error(f"Memory error at epoch {epoch}: {str(e)}")
