...
class LiFePO4Parameters:
    ...
    def update_from_dimensionless(self, W_dim=None, κ_dim=None, M_dim=None):
        """Update physical parameters from dimensionless values"""
        if W_dim is not None:
            self.Ω_density = W_dim * self.energy_scale
            self.Ω = self.Ω_density * self.V_m
            
        if κ_dim is not None:
            self.κ = κ_dim * self.energy_scale * self.length_scale**2
            
        if M_dim is not None:
            # M_dim = M_physical / (energy_scale * time_scale)
            M_physical = M_dim * self.energy_scale * self.time_scale
            self.D_b = M_physical * self.energy_scale  # Approximate relation
...
class LiFePO4Simulation:
    ...
    def __init__(self, nx=256, ny=256, dx=1.0, dt=0.01):
        ...
        # Dimensionless parameters (for easier adjustment)
        self.W_dim = 1.0  # Dimensionless double-well height
        self.κ_dim = 2.0  # Dimensionless gradient coefficient
        self.M_dim = 1.0  # Dimensionless mobility
        ...
        # Update physical parameters from dimensionless
    ...
    def update_physical_params(self):
        """Update physical parameters from dimensionless values"""
        self.params.update_from_dimensionless(
            W_dim=self.W_dim,
            κ_dim=self.κ_dim,
            M_dim=self.M_dim
        )
    
    def set_dimensionless_params(self, W_dim=None, κ_dim=None, M_dim=None):
    ...
    def run_step(self):
        ...
        M_effective = self.M_dim * self.params.energy_scale * self.params.time_scale
    ...
    def get_statistics(self):
        ...
        if mean_c > 0 and mean_c < 1:
            μ_Li = self.params.Ω * (1 - 2 * mean_c) + R * self.params.T * np.log(mean_c / (1 - mean_c))
            U_ocv = U0 - μ_Li / F
        else:
            U_ocv = U0
        
        # Calculate characteristic length and time scales
        domain_size_nm = self.nx * self.dx * self.params.length_scale * 1e9
        interface_width_nm = np.sqrt(self.params.κ / self.params.Ω_density) * 1e9
        ...
        return {
            ...
            'κ_dim': self.κ_dim,
            'M_dim': self.M_dim,
            'Ω_physical': self.params.Ω / 1000,  # in kJ/mol
            'κ_physical': self.params.κ,
            'D_b_physical': self.params.D_b,
        }
    ...
    **Cahn-Hilliard equation** with physical parameters from literature.
    ...
    with st.sidebar:
        ...
        with st.expander("📊 Physical Parameters", expanded=False):
            ...
            **Ω (physical):** {stats['Ω_physical']:.1f} kJ/mol  
            **κ (physical):** {stats['κ_physical']:.2e} J/m  
            **D_b (physical):** {stats['D_b_physical']:.2e} m²/s  
            **Interfacial Energy:** {sim.params.σ:.3f} J/m²  
            **Interface Thickness:** {stats['interface_width_nm']:.1f} nm  
        ...
        with col2:
            if st.button("▶️ Run Steps", use_container_width=True, key="run_steps"):
        ...
        if st.button("🔄 Apply Initialization", use_container_width=True, key="apply_init"):
        ...
        # Regular solution parameter Ω (dimensionless)
        W_dim = st.slider(
            "Double-well height (W_dim)", 
            0.1, 10.0, float(sim.W_dim), 0.1,
            help="Controls barrier between phases. Higher = sharper interfaces, stronger phase separation"
        )
        
        # Gradient energy coefficient κ
        κ_dim = st.slider(
            "Gradient coefficient (κ_dim)", 
            0.1, 10.0, float(sim.κ_dim), 0.1,
            help="Controls interface width and energy. Higher = wider interfaces, higher interfacial energy"
        )
        
        # Mobility M
        M_dim = st.slider(
            "Mobility (M_dim)", 
            0.01, 10.0, float(sim.M_dim), 0.01,
            help="Controls kinetics. Higher = faster phase separation"
        )
        
        # Update parameters if changed
        if (W_dim != sim.W_dim or κ_dim != sim.κ_dim or M_dim != sim.M_dim):
            sim.set_dimensionless_params(W_dim=W_dim, κ_dim=κ_dim, M_dim=M_dim)
        ...
        avg_concentration = st.slider("Average Li Concentration", 0.1, 0.9, 0.5, 0.01,
                                     help="Overall lithium content in the particle")
        
        if st.button("Update Concentration", use_container_width=True, key="update_conc"):
            sim.initialize_random(c0=avg_concentration, noise_amplitude=0.05)
            st.rerun()
    ...
    with tab3:
        ...
        if len(sim.history['time']) > 1:
            ...
            # Concentration evolution - FIXED: Changed 'purple-' to color='purple'
            ...
            # Phase fractions - FIXED: Corrected matplotlib syntax
            ...
            # Standard deviation (measure of phase separation) - FIXED: Changed 'purple-' to color='purple'
            axes[1, 0].plot(sim.history['time'], sim.history['std'], color='purple', linewidth=2)
            axes[1, 0].set_xlabel("Time (s)")
            axes[1, 0].set_ylabel("Standard Deviation")
            axes[1, 0].set_title("Concentration Fluctuations")
            axes[1, 0].grid(True, alpha=0.3)
            
            # Voltage curve - FIXED: Changed 'orange-' to color='orange'
            # Calculate voltage history
            voltages = []
            for mean_c in sim.history['mean']:
                if mean_c > 0 and mean_c < 1:
                    μ_Li = sim.params.Ω * (1 - 2 * mean_c) + R * sim.params.T * np.log(mean_c / (1 - mean_c))
                    U_ocv = 3.42 - μ_Li / F
                else:
                    U_ocv = 3.42
                voltages.append(U_ocv)
            
            axes[1, 1].plot(sim.history['time'], voltages, color='orange', linewidth=2)
            ...
            st.info("Run some simulation steps to see statistics history.")
    
    # Parameter effects explanation
    with st.expander("📖 Parameter Effects Guide", expanded=False):
        col1, col2, col3 = st.columns(3)
        
        with col1:
            st.markdown("""
            ### W (Double-well Height)
            **Physical meaning:** Energy barrier between phases
            
            **Effects:**
            - Higher W → Sharper interfaces
            - Higher W → Stronger phase separation
            - Higher W → Faster phase separation
            - Higher W → Larger driving force
        ...
        with col2:
            st.markdown("""
            ### κ (Gradient Coefficient)
            **Physical meaning:** Interfacial energy contribution
            
            **Effects:**
            - Higher κ → Wider interfaces
            - Higher κ → Higher interfacial energy
            - Higher κ → Slower phase separation
            - Higher κ → Smoother interfaces
        ...
        with col3:
            st.markdown("""
            ### M (Mobility)
            **Physical meaning:** Kinetics of phase separation
            
            **Effects:**
            - Higher M → Faster evolution
            - Higher M → Larger domain growth rate
            - Higher M → Faster coarsening
            - Higher M → Quicker equilibrium
    ...
    # Auto-run option
    st.sidebar.divider()
    auto_run = st.sidebar.checkbox("Auto-run simulation", value=False, key="auto_run")
    auto_speed = st.sidebar.slider("Auto-run speed (steps/sec)", 1, 100, 10, key="auto_speed")
    
    if auto_run:
        placeholder = st.empty()
        stop_button = st.sidebar.button("Stop Auto-run", key="stop_auto")
    ...
    with st.expander("📚 Theory and Explanation", expanded=False):
        ...
        - `Ω` = regular solution parameter (controls phase separation strength)
        ...
        We use dimensionless parameters for easier adjustment:
        
        - **W_dim** = Ω_density / energy_scale (controls barrier between phases)
        - **κ_dim** = κ / (energy_scale × length_scale²) (controls interface width)
        - **M_dim** = D_b / (energy_scale × time_scale) (controls kinetics)
        
        ### Physical Interpretation:
        
        1. **Spinodal Decomposition**: When W_dim > 2RT/V_m, the system is unstable
           and spontaneously separates into two phases.
        
        2. **Interface Properties**: The interface thickness scales as √(κ/Ω_density).
           Higher κ gives wider interfaces, higher Ω gives sharper interfaces.
        
        3. **Domain Coarsening**: Larger domains grow at the expense of smaller ones
           to reduce interfacial energy (scaling as t¹/³).
        
        ### Applications to Battery Materials:
        
        - **Voltage Plateau**: The flat voltage during phase separation corresponds
          to the two-phase coexistence region.
        - **Rate Capability**: Higher mobility M allows faster phase separation,
          enabling faster charging/discharging.
        - **Size Effects**: Smaller particles suppress phase separation due to
          increased surface-to-volume ratio.
        - **Anisotropy**: LiFePO₄ shows strong anisotropic diffusion, affecting
          phase boundary morphology.
        """)
