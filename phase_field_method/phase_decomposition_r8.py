...
class PhysicalScalesWithElectrostatics:
    """Physical scales with electrostatics for LiFePO₄"""
    ...
    def __init__(self):
        ...
        print(f"Physical scales with electrostatics:")
    ...
    def calculate_debye_length(self):
        ...
        # For a binary electrolyte approximation
...
@njit(fastmath=True, parallel=True)
def update_concentration_with_electrostatics(c, phi, dt, dx, kappa, M, D, A, B, C, 
                                           z, F, R, T):
    """
    Update concentration using extended Cahn-Hilliard with electrostatics:
    ∂c/∂t = ∇·[M ∇μ + (D * z * F * c / (R * T)) ∇φ]
    where μ = df/dc - κ∇²c + zFφ
    """
    nx, ny = c.shape
    ...
    # Einstein relation: D = (R*T) * M / c_ref (approximate)
    # Use c instead of c_ref for concentration-dependent mobility
    c_safe = np.maximum(1e-6, c)  # Avoid division by zero
    D_eff = M * R * T / c_safe
    ...
    # Update concentration: ∂c/∂t = ∇·(-flux) = -∇·flux
    c_new = c - dt * div_flux
...
class ElectrochemicalPhaseFieldSimulation:
    ...
    def __init__(self, nx=256, ny=256, dx=1.0, dt=0.1):
        ...
        # Physical scales with electrostatics
        self.scales = PhysicalScalesWithElectrostatics()
    ...
    def initialize_lithiation(self):
        ...
        # Add LiFePO₄ seed at left boundary (simulating insertion from electrolyte)
        seed_width = self.ny // 10
        for j in range(self.ny):
            if abs(j - self.ny//2) < seed_width:
                self.c[:10, j] = self.scales.c_beta
        
        # Apply electric potential gradient (negative at left for Li⁺ insertion)
        self.phi = np.zeros_like(self.c)
        for i in range(self.nx):
            self.phi[i, :] = -0.1 * (i / self.nx)  # Linear gradient
    ...
    def initialize_delithiation(self):
        ...
        # Apply electric potential gradient (positive at left for Li⁺ extraction)
        self.phi = np.zeros_like(self.c)
        for i in range(self.nx):
            self.phi[i, :] = 0.1 * (i / self.nx)  # Linear gradient
    ...
    def update_history(self):
        ...
        # Assume voltage is potential at right minus left boundary
    ...
    def run_step(self):
        ...
        # Update concentration with electrostatics
        self.c = update_concentration_with_electrostatics(
            self.c, self.phi, self.dt, self.dx,
            self.kappa_dim, self.M_dim, self.scales.D_b,
    ...
    def compute_free_energy_density(self):
        ...
        # Add gradient energy (approximate)
    ...
    def get_statistics(self):
        ...
        stats = {
            ...
            'std_c': np.std(self.c),
            'x_Li': np.mean(self.c),  # Simplified
...
def main():
    ...
    # Title
    st.title("⚡ LiFePO₄ Electrochemical Phase Field with Electrostatics")
    st.markdown("""
    ...
    if 'sim' not in st.session_state:
        st.session_state.sim = ElectrochemicalPhaseFieldSimulation(nx=256, ny=256, dx=1.0, dt=0.01)
    ...
    with tab3:
        ...
        with col2:
            ...
            Ex_subsampled = sim.Ex[::subsample, ::subsample].T
            Ey_subsampled = sim.Ey[::subsample, ::subsample].T
            
            fig3b, ax3b = plt.subplots(figsize=(6, 5))
            ax3b.quiver(X, Y, Ex_subsampled, Ey_subsampled, 
                       np.sqrt(Ex_subsampled**2 + Ey_subsampled**2),
    ...
    with st.expander("📚 Governing Equations with Electrostatics", expanded=True):
        st.markdown("""
        ### Extended Cahn-Hilliard Equation with Electrostatics
        
        The model now couples **phase separation** with **electrostatics**:
        
        1. **Modified Chemical Potential**:
           ```
           μ = ∂f/∂c - κ∇²c + zFφ
           ```
           where φ is the electric potential from Poisson's equation.
        
        2. **Nernst-Planck Flux**:
           ```
           J = -M∇μ - (D*z*F*c/(RT))∇φ
           ```
           Includes both **diffusion** (first term) and **migration** (second term).
        
        3. **Conservation Equation**:
           ```
           ∂c/∂t = -∇·J = ∇·[M∇μ + (D*z*F*c/(RT))∇φ]
           ```
        ...
        4. **Poisson Equation**:
           ...
           ∇²φ = -F/ε * (c - c_ref)
           ```
           where ε is permittivity and c_ref is reference concentration.
        
        ### Physical Interpretation:
        
        - **Electric Field Drives Li⁺ Migration**: During charging (lithiation), 
          negative potential attracts Li⁺ into the particle
        - **Double Layer Formation**: Charge accumulates at phase boundaries, 
          creating electric field peaks
        - **Debye Screening**: Electric fields are screened over ~0.3 nm 
          (very short range in LiFePO₄)
        - **Voltage**: Calculated from potential difference across the particle
        
        ### Key Effects Captured:
        
        1. **Accelerated Phase Boundaries**: Electric field accelerates 
           Li⁺ transport during charging/discharging
        
        2. **Asymmetric Charging/Discharging**: Different electric field 
           directions lead to different phase boundary morphologies
        
        3. **Double Layer Capacitance**: Charge accumulation at interfaces 
           affects voltage response
        
        4. **Concentration-Potential Coupling**: Phase separation affects 
           electric field distribution and vice versa
        
        ### Numerical Implementation:
        
        - **Poisson Solver**: Jacobi iteration with periodic BCs
        - **Coupled Updates**: Solve Poisson → Update concentration
        - **Stability**: Small time steps (Δt ≈ 0.01) required for stability
        - **Conservation**: Total Li content conserved (periodic BCs)
        """)
