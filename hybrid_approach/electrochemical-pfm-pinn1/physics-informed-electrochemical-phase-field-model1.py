...
class LiFePO4AssimilationPINN(nn.Module):
    """Lightweight PINN for concentration field reconstruction at a single time slice."""
    def __init__(self, Lx, Ly):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(64, 64), nn.Tanh(),
            nn.Linear(64, 1), nn.Sigmoid()  # Output c ∈ [0,1]
        )
    ...
    def forward(self, x, y):
        ...
        x_norm = x / self.Lx
        y_norm = y / self.Ly
        input_tensor = torch.cat([x_norm, y_norm], dim=1)
        return self.net(input_tensor)

def physics_loss(model, x, y, phi_fdm, kappa, A, B, C, z, F):
    """Physics loss: Penalize variance in total electrochemical potential."""
    ...
    # Chemical potential
    mu_chem = 2 * A * c + 3 * B * c**2 + 4 * C * c**3 - kappa * lap_c
    
    # Total electrochemical potential
    mu_total = mu_chem + z * F * phi_fdm
    
    # Penalize variance (approximates μ ≈ constant)
    mu_var = torch.var(mu_total)
    return mu_var
...
class ElectrochemicalPhaseFieldSimulation:
    ...
    def generate_sparse_synthetic_data(self, sparsity=0.0015, noise_amplitude=0.05):
        """Generate sparse synthetic observations from current c field with noise."""
        num_points = int(self.nx * self.ny * sparsity)  # e.g., 0.15% → ~100 points for 256x256
        indices = np.random.choice(self.nx * self.ny, num_points, replace=False)
        x_idx, y_idx = np.divmod(indices, self.ny)
        c_obs = self.c[x_idx, y_idx] + noise_amplitude * np.random.randn(num_points)
        c_obs = np.clip(c_obs, 0.0, 1.0)
        return x_idx.astype(float), y_idx.astype(float), c_obs  # As floats for Torch

    def perform_pinn_correction(self, x_obs, y_obs, c_obs, epochs=1000, lr=0.001, lambda_phys=1.0):
        """Train PINN on sparse observations and reconstruct full field."""
        model = LiFePO4AssimilationPINN(self.nx, self.ny)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        
        # Convert observations to tensors
        x_tensor = torch.tensor(x_obs).float().unsqueeze(1)
        y_tensor = torch.tensor(y_obs).float().unsqueeze(1)
        c_obs_tensor = torch.tensor(c_obs).float().unsqueeze(1)
        
        # Use FDM's phi as fixed input (sample at obs points)
        phi_fdm_obs = torch.tensor(self.phi[x_obs.astype(int), y_obs.astype(int)]).float().unsqueeze(1)
        
        for epoch in range(epochs):
            optimizer.zero_grad()
            
            # Data loss
            c_pred = model(x_tensor, y_tensor)
            data_loss = nn.MSELoss()(c_pred, c_obs_tensor)
            
            # Physics loss (at observation points for efficiency)
            phys_loss = physics_loss(model, x_tensor, y_tensor, phi_fdm_obs,
                                     self.kappa_dim, self.A, self.B, self.C, self.scales.z, self.scales.F)
        ...
        x_grid = torch.arange(0, self.nx).float().unsqueeze(1)
        y_grid = torch.arange(0, self.ny).float().unsqueeze(1)
        X, Y = torch.meshgrid(x_grid.squeeze(), y_grid.squeeze(), indexing='ij')
        with torch.no_grad():
            c_pinn = model(X.flatten().unsqueeze(1), Y.flatten().unsqueeze(1)).reshape(self.nx, self.ny)
        
        self.c = c_pinn.numpy()  # Update simulation state
        print(f"PINN correction applied at t={self.time_phys:.2e} s")

    def run_steps_with_corrections(self, n_steps, correction_interval_steps=1000, use_synthetic_data=True, sparsity=0.0015, noise=0.05):
        """Run steps with periodic PINN corrections."""
        for i in range(n_steps):
            self.run_step()
            if (i + 1) % correction_interval_steps == 0:
                if use_synthetic_data:
                    x_obs, y_obs, c_obs = self.generate_sparse_synthetic_data(sparsity, noise)
                else:
                    # Placeholder for real data; load from file or UI
                    pass
                self.perform_pinn_correction(x_obs, y_obs, c_obs)
...
def main():
    ...
    with st.sidebar:
        ...
        with col_time2:
            if st.button("▶️ Run Steps", use_container_width=True):
                with st.spinner("Running..."):
                    if 'enable_pinn' in locals() and enable_pinn:
                        sim.run_steps_with_corrections(steps, correction_interval_steps=correction_interval,
                                                       use_synthetic_data=use_synthetic, sparsity=sparsity, noise=noise_amp)
                    else:
                        sim.run_steps(steps)
                    st.rerun()
        ...
        st.divider()
        st.subheader("🧠 PINN Data Assimilation")
        enable_pinn = st.checkbox("Enable PINN Corrections", value=False)
        if enable_pinn:
            correction_interval = st.number_input("Correction Interval (steps)", 100, 5000, 1000)
            use_synthetic = st.checkbox("Use Synthetic Sparse Data", value=True)
            sparsity = st.slider("Sparsity (% coverage)", 0.1, 1.0, 0.15) / 100
            noise_amp = st.slider("Observation Noise Amplitude", 0.0, 0.2, 0.05)
            lambda_phys = st.slider("Physics Loss Weight (λ)", 0.1, 10.0, 1.0)
