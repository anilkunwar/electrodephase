...
# =====================================================
# 2. SYNTHETIC OBSERVATION GENERATOR
# =====================================================

@st.cache_resource
class SyntheticObservationGenerator:
    """Generate synthetic experimental observations from FDM simulation"""
    ...
    def generate_sparse_observations(
        ...
        """
        Generate synthetic sparse observations from concentration field
        """
        if seed is not None:
            np.random.seed(seed)
        ...
        with st.expander("⚙️ Simulation Setup", expanded=True):
            ...
            if st.button("🔄 Initialize Simulation", use_container_width=True):
                with st.spinner("Initializing simulation..."):
                    ...
                    if init_type == "Random":
                        st.session_state.hybrid_system.sim.initialize_random()
                    elif init_type == "Lithiation (Charge)":
                        st.session_state.hybrid_system.sim.initialize_lithiation()
                    else:
                        st.session_state.hybrid_system.sim.initialize_delithiation()
    ...
    with tab4:
        ...
        with col1:
            ...
            with st.expander("⚡ Governing Equations", expanded=False):
                ...
                **PINN enforces:**
                ```
                L_physics = Var(μ + zFφ) → minimized
                ```
                """)
            
            with st.expander("🔬 Experimental Data Sources", expanded=False):
                st.markdown("""
                | Technique | Coverage | Noise | Pattern |
                |-----------|----------|-------|---------|
                | **In-situ Microscopy** | 10-20% | 5-10% | Random |
                | **XRD Mapping** | 5-10% | 3-5% | Grid |
                | **Tomography** | 15-20% | 4-6% | Lines |
                | **AFM/SEM** | 8-12% | 6-8% | Random |
                ...
                - **Quality Control**: Detect defects via assimilation anomalies  
        ...
        with col2:
            ...
            <tr><td><strong>Data Efficiency</strong></td><td>5-20% coverage</td></tr>
            <tr><td><strong>Computational Overhead</strong></td><td>+20-30%</td></tr>
            <tr><td><strong>Real-time Capable</strong></td><td>Yes</td></tr>
            <tr><td><strong>Degradation Tracking</strong></td><td>Automatic</td></tr>
            </table>
            </div>
            """, unsafe_allow_html=True)
            ...
            <ol>
            <li>Set grid size & C-rate</li>
            <li>Initialize simulation</li>
            <li>Configure observations</li>
            <li>Set PINN parameters</li>
            <li>Define schedule</li>
            <li>Run assimilation</li>
